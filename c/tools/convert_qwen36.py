...
def quantize_row_grouped(w: "torch.Tensor", bits: int, gs: int):
    ...
    per row. Returns (q int8 [O,I], scales f32 [O, ceil(I/gs)])."""
    qmax = (1 << (bits - 1)) - 1
...
def make_merged(gate, up, down, ebits, gs=0):
    ...
    def q(t):
        if gsz:
            qt, s = quantize_row_grouped(t, ebits, gsz)
...
def main():
    ...
    ap.add_argument("--gs", type=int, default=0,
                    help="Group size for expert scales (e.g. 64). 0 = per-row (default). "
                         "Group-scaled containers set expert_gs in meta; the engine picks matmul_q_gs.")
    ...
    if full_idx:
        ...
        meta["expert_gs"] = args.gs   # 0 = per-row; >0 = group size along input dim
