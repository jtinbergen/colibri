 ...
 * (gs64 expert containers). Scale row layout: [O][I/gs] row-major. */
 ...
 * Windows: set stdout/stdin to binary BEFORE the READY sentinel or the CRT
 * rewrites the trailing \n as \r\n and the gateway never matches it -> the
 * session hangs forever (#748, fixed in #749). We inline the fix here because
 * #749's shared helper was not in the base when this was written; switch to
 * coli_serve_binary_mode() from compat.h once #749 lands. */
#ifdef _WIN32
#include <io.h>
#endif
...
int main(int argc, char **argv) {
    ...
    gpu_probe();   /* transparent: use GPU if a Vulkan compute device exists, else CPU */
